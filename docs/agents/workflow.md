...
If production or package deployment is added, the candidate must be identified by its exact `main` commit plus immutable version/artifact where available. A changed candidate invalidates prior approval. A deployment-system approval gate must prevent an unapproved candidate from deploying; agent-controlled merge must not bypass that gate. A human must approve the exact candidate before deployment starts.
