...
Default activation policy: a `shipyard` ticket receives triage before implementation; implementation requires both `shipyard` and an unconflicted `ready-for-agent` state. Other triage states block the attempt and remove activation. After evidence from an initial representative trial, permit automatic implementation of clearly defined, low-risk classes. Auth, billing, migrations, destructive operations, and workflow/security-policy changes receive explicit owner involvement. External intake must not grant the reporter control over credentials, tools, or execution policy.
