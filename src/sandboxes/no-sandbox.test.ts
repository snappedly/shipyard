...
describe("noSandbox", () => {
  ...
  describe("handle", () => {
    ...
    itPosix("exec passes env vars to spawned processes", async () => {
      ...
      const handle = await provider.create({
        worktreePath: process.cwd(),
        env: { MY_TEST_VAR: "shipyard_test_value" },
      });

      const result = await handle.exec("echo $MY_TEST_VAR");
      expect(result.stdout.trim()).toBe("shipyard_test_value");
    });
    ...
    itPosix("interactiveExec terminates the process when aborted", async () => {
      const provider = noSandbox();
      const handle = await provider.create({
        worktreePath: process.cwd(),
        env: {},
