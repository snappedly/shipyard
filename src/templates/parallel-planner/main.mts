// Parallel Planner: plan activated work, implement independent issues in
// parallel, then validate and publish each issue branch for human review.
...
for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
  ...
  // The selected template keeps its parallel execution stage. Every worker is
  // assigned one ticket and returns evidence to this coordinator.
  ...
  let completed = 0;
  // The former target-branch merge phase now validates each completed branch
  // and publishes its own PR. It never merges into the configured target.
  for (const [index, outcome] of settled.entries()) {
    ...
    try {
      ...
      if (!merged.completionSignal || !finalEvidence)
        throw new Error(`Final checks or review incomplete for #${issue.id}`);
