...
If incomplete, return `<handoff>` with one line each for `Facts:`, `Checks:` (or why checks could not run), and `Blocker:`. Omit the completion marker. After a verified commit, report scope, `Checks: <commands and results>`, and limitations:
