...
describe("output.maxRetries entry-time validation", () => {
  ...
  it("throws when maxRetries > 0 and the agent provider lacks session resumption", async () => {
    await expect(
      run({
  ...
  it("enforces session resumption for string output retries", async () => {
    await expect(
      run({
        agent: nonResumableAgent,
        sandbox: testSandbox,
