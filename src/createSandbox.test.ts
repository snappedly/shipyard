...
const testCodexProvider = codex("test-model");
 ...
 * Uses testIsolated() as a base and wraps exec to intercept agent commands.
 */
...
describe("createSandbox", () => {
  ...
  it("sandbox.exec() defaults cwd to the sandbox repo path", async () => {
    ...
    try {
      ...
      expect(result.stdout.trim()).toBe(await realpath(sandbox.worktreePath));
    } finally {
      await sandbox.close();
      await rm(hostDir, { recursive: true, force: true });
    }
  ...
  it("sandbox.exec() allows the caller to override the default cwd", async () => {
    ...
    try {
      ...
      expect(result.stdout.trim()).toBe(await realpath(hostDir));
    } finally {
      await sandbox.close();
      await rm(hostDir, { recursive: true, force: true });
    }
