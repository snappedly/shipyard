  ...
  Effect.gen(function* () {
    ...
    const [hostGitName, hostGitEmail] = yield* Effect.promise(async () => {
      const [nameResult, emailResult] = await Promise.all([
        execHostGit(["config", "user.name"], hostRepoDir)
          .then((stdout) => stdout.trim())
          .catch(() => ""),
        execHostGit(["config", "user.email"], hostRepoDir)
          .then((stdout) => stdout.trim())
          .catch(() => ""),
      ]);
    ...
    yield* display.taskLog("Setting up sandbox", (message) =>
      Effect.gen(function* () {
        ...
        if (hostGitName) {
          yield* execOkWithGitTimeout(
            sandbox,
            `git config --global user.name ${shellQuote(hostGitName)}`,
            gitSetupTimeoutMs,
          );
        }
        if (hostGitEmail) {
          yield* execOkWithGitTimeout(
            sandbox,
            `git config --global user.email ${shellQuote(hostGitEmail)}`,
            gitSetupTimeoutMs,
          );
        }
    ...
    if (hostCurrentBranch !== null) {
      ...
      if (!options.keepSourceBranch) {
        if (options.applyToHost) {
          // Sync-out leaves the host worktree attached to the source branch.
          // Detaching only the sandbox cannot release that host branch.
          yield* Effect.tryPromise({
            try: () =>
              execHostGit(["checkout", "--detach"], hostSideWorktreePath),
            catch: (error) =>
              new SyncError({
                message: `Failed to detach host worktree: ${String(error)}`,
              }),
          });
        } else {
      ...
      if (hasNewCommits) {
        ...
        yield* display.taskLog(`Merging to ${hostCurrentBranch}`, () =>
          Effect.tryPromise({
            try: async () => {
              try {
                await execHostGit(["merge", "--", resolvedBranch], hostRepoDir);
              } catch {
                throw new Error(
        ...
        Effect.promise(async () => {
          try {
            const stdout = await execHostGit(
              [
                "rev-list",
                `${baseHead}..refs/heads/${targetBranch}`,
                "--reverse",
              ],
              hostRepoDir,
            );
